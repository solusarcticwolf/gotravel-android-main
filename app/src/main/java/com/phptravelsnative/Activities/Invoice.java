...
public class Invoice extends Drawer {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        }else if(intent.getStringExtra("check_type").equals("flights"))
        {
            TravelPort cabin_class=intent.getParcelableExtra("cabin_class");
            setupViewPager(viewPager,cabin_class,intent.getStringExtra("id"));
        }
    ...
    private void setupViewPager(ViewPager viewPager, TravelPort cabin_class, String id) {

        ViewPagerAdapter adapter = new ViewPagerAdapter(getSupportFragmentManager());

        if(!sharedPreferences.getBoolean("Check_Login",true)) {
            adapter.addFragment(Guest.newInstance(id,cabin_class), getString(R.string.guest));
            adapter.addFragment(Login_Fragment.newInstance(id,cabin_class), getString(R.string.login));
        }
        if(sharedPreferences.getBoolean("coupons",false)) {
            adapter.addFragment(Coupon.newInstance(id,cabin_class), getString(R.string.coupon));
        }
        viewPager.setAdapter(adapter);
